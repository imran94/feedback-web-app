...
async function fetchPost(postId) {
    ...
    const res = await utils.networkRequest(`/feedback/${postId}`)
    if (res.status === 200) {
...
async function submitFeedback() {
    ...
    try {
        const response = await utils.networkRequest(endpoint, method, post.value)
        if (response.status === 200) {
            const mPost = await response.json()
            Swal.fire({
        ...
        <div class="body">
            ...
            <form @submit.prevent="submitFeedback">
                ...
                <div class="m-form-group">
                    ...
                    <input v-model="post.title" type="text" id="titleInput" class="form-control" maxlength="250" required
                        :disabled="isLoading">
                </div>
