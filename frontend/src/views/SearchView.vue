...
async function search() {
    ...
    console.log('categories', searchCategories.value.filter(c => c.isSelected).map(c => c.name))
...
onMounted(() => {
    ...
    searchCategories.value = utils.getCategories().map(category => {
        return { name: category, isSelected: false }
    })
})
</script>
...
<template>
    ...
    <div class="body">
        <form @submit.prevent="search" class="search-form">
            ...
            <input id="searchInput" v-model="searchTerm" type="search" class="form-control search-field" maxlength="250"
                required>
            ...
            <div class="search-filter-group">
                <div class="category" v-for="category in searchCategories" :key="category.name">
                    <input class="form-check-input" type="checkbox" :id="category.name" v-model="category.isSelected"
